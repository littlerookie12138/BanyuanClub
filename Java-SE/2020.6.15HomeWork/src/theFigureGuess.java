...
//			}
        ...
        int tempLen = getResult(result, num);//第一次计算
        ...
        while (!isSingle) {
            ...
            tempResult = arrCopy(tempResult, countResult(num));
            System.out.println("==========未排序前==========");
            for (int i = 0; i < num; i++) {
                if (tempResult[i] != 0) {
...
//			System.out.println(isThree);
            ...
            //如果出现了再来一波
            while (isThree == true) {
                System.arraycopy(countResult(num), 0, tempResult, 0, num);
                ...
                if (isThree == false) {
                    ...
                    if (tempLen == 1) {
                        ...
                        isThree = isReapeat(tempResult, num);
                    }
                    break;
                }
            }
            tempLen = getResult(tempResult, num);
            if (tempLen == 1) {
                isSingle = true;
            } else {
                //如果长度还没剩下一个，继续运行
