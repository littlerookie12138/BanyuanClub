...
答案：java中不能使用static修饰顶级类，只有内部类可以为static
```
...
答案：不能，非静态内部类在加载时依赖于外部类对象的简历，因此不可以有静态成员
```
#### 5.
```
